...
def render_sweep_html(rungs: list[dict], report_context: dict,
    ...
    quota_code = quota.get("quota_status") or "UNKNOWN"
    quota_label = {
        "HEADROOM_UNESTABLISHED_SHORT_WINDOW":
            "Headroom not established (short run)",
        "HEADROOM_UNESTABLISHED": "Headroom not established",
        "LIMITED": "Quota rejection observed",
        "NOT_LIMITED": "No quota rejection observed",
        "UNKNOWN": "Quota state unknown",
    }.get(quota_code, str(quota_code).replace("_", " ").title())
...
.card strong{{overflow-wrap:anywhere;word-break:break-word}}
</style>
</head>
...
<article class='card'><div class='label'>Quota evidence</div><strong>{esc(quota_label)}</strong><small>{esc(quota.get('http_429_count', 0))} HTTP 429; local guard {esc(local.get('status') or 'not recorded')}</small></article>
...
class SweepArtifacts:
    ...
    def add_rung(self, rate: float, run_dir: str | Path,
        ...
        if expected_summary is not None and summary != expected_summary:
            ...
            raise ValueError(
                "runner summary disagrees with manifest-bound summary.json "
                f"at top-level fields {differing}: {d}")
        identity = d.stat()
